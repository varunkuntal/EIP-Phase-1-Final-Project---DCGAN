...
def load_celebA(img_dim, image_data_format):
    ...
    with h5py.File("/content/DeepLearningImplementations/GAN/data/processed/CelebA_%s_data.h5" % img_dim, "r") as hf:
...
def plot_generated_batch(X_real, generator_model, batch_size, noise_dim, image_data_format, noise_scale=0.5):
    ...
    if Xr.shape[-1] == 1:
        ...
        plt.imshow(Xr)
    plt.savefig("/content/DeepLearningImplementations/GAN/figures/current_batch.png")
    plt.clf()
    plt.close()

...
# Utils
sys.path.append("/content/DeepLearningImplementations/Colorful/src/utils/")
import batch_utils
...
def train(**kwargs):
    ...
    q_ab = np.load("/content/DeepLearningImplementations/Colorful/data/processed/pts_in_hull.npy")
    nb_q = q_ab.shape[0]
    # Fit a NN to q_ab
    ...
    prior_factor = np.load("/content/DeepLearningImplementations/Colorful/data/processed/CelebA_%s_prior_factor.npy" % img_size)
    ...
    for f in glob.glob("/content/DeepLearningImplementations/Colorful/figures/*.png"):
        os.remove(f)
    ...
    try:
        ...
        plot_model(color_model, to_file='/content/DeepLearningImplementations/Colorful/figures/colorful.png', show_shapes=True, show_layer_names=True)
